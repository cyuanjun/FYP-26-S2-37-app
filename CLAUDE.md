...
**Current state: the Flutter app is built and running** (Android emulator + iOS simulator) against a live Supabase backend — vertical slice complete plus onboarding, AI plans (live OpenAI), My Plans + Plan Detail, connected devices, the full Profile cluster, the **Social cluster** (feed + likes/comments, mutual friends + user profiles, challenges with live leaderboards), and the **Experts marketplace** (#6 browse → request → deliverable → review, simulated payment; expert accounts get a role-swapped request inbox on the same tab); 168 tests. A **local Supabase stack** exists (`cd app && supabase start`, ports 55321-9; target it with `--dart-define`s) — new backend work is tested locally first. `docs/` holds the planning/design/reference material; **[docs/STATUS.md](docs/STATUS.md) is the up-to-date state**. The design decisions below are **already made** — implement to them; don't reopen them without being asked.
