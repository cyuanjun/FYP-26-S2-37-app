...
| B2 | AI provider → **OpenAI primary, Gemini fallback** (in code + repo docs; both edge functions keep the Gemini path). **Note (13 Jun):** the **PTD** describes the AI fallback as **OpenAI → rule-based stub only** (Gemini omitted, per team decision to simplify the deliverable). Code/repo intentionally retain Gemini — this is a deliberate doc-vs-code divergence, not a defect. | §9.3 | Confirms the "OpenAI/Gemini" listing; PTD wording simplified. |
...
| C2a | **Admin portal realized as a WEB app** (decided 8 Jul 2026), not a mobile-app track — the TDM §7 wireframes and SRS admin use cases (US53–US64) stay valid for content/flows, but the platform column changes; PRD/SRS/TDM should say "admin web portal" where they imply the mobile app. Mobile app keeps role-based redirects only. | PRD §4, SRS §US53–64, TDM §7 | Follow-up edits pending. |
| C2b | **US21 rest alerts realized Premium-only** (decided 8 Jul, at build time): the WBS plans them Premium-only while the SRS words US21 under the Free user — the build follows the WBS (rest alert = Premium perk, pitched on #16). SRS US21 wording needs a Premium qualifier. | SRS §US21 | Divergence flagged in user-stories.md §"no WBS work package"; resolved toward WBS. |
...
| C8 | **US13 manual workout entry DESCOPED — removed** (built 9 Jul, cut shortly after). A free-text manual entry ran the same `end_workout_session` RPC as a captured session, so users could fabricate workouts and farm XP / levels / streaks with no sensor evidence — undermining the gamification the app is graded on. Decision: **all progress must come from a captured session** (phone GPS, freeform, or wearable). Code removed (`ManualEntryScreen`, `LogManualWorkout` control, tests + Train-screen entry point); status in user-stories.md set to ⬜ descoped. SRS US13 should be marked **out of scope for the FYP build** with this integrity rationale (keep the story text; add a scope note). | SRS §US13 | Engineering decision leads; integrity of XP/level/streak > convenience of manual logging. |
| C9 | **Challenge join codes added to US25** (net-new after PTD/PUM assembly). Every challenge gets a unique, server-assigned, ambiguity-free 6-char code (migration `20260713100000_challenge_join_code.sql`): users enter a code in the Challenges-tab search field to open + join a challenge, or reveal/share it from the detail's invite popup (`FindChallengeByCode` control). If the deliverables are revised, add this to the PUM social/challenges walkthrough; the US25 story text still holds (join codes are a means to the "join challenges" story). | SRS §US25 / PUM social walkthrough | Engineering decision leads; feature added post-assembly. |
