...
> - **Marketplace controls built (7 Jul):** client `ToggleFollowExpert` (bookmark array on profiles), `RequestService` (§5.6, price snapshot), `SubmitReview` (RPC-gated); expert `AcceptServiceRequest`/`DeclineServiceRequest`/`SendDeliverable`/`CompleteServiceRequest` — realized as SECURITY DEFINER RPCs + a grants lockdown (direct status writes revoked). **Realization note:** the expert side ships as a role-swapped Experts tab (request inbox + deliverable composer), not the #20–24 portal (no spec files; deliberate minimal realization). `PublishService` remains pending (services are seed-provisioned).
