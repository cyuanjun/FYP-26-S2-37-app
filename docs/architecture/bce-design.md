...
> - **Sequence-diagram note:** §5.4 (share) and §5.5 (join challenge) now have running counterparts; like/comment/follow/create-challenge flows follow the same Boundary→Control→Gateway shape and are SEQ-logged with use-case ids `toggle-like`, `add-comment`, `follow-user`, `join-challenge`, `create-challenge`, `view-feed`, `view-challenges`.
