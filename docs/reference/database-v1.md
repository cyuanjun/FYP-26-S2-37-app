...
> ⚠️ **The schema of record is now the TDM v3.0 §8 ERD** (team-approved, 5 Jun 2026). This file is the **working source** that predates it (built from the React mock). Before generating DDL, **align this to the TDM §8 ERD** — the two are ~1:1; the TDM settles the open questions (`ExpertReview` kept; expert layer = `ExpertService → ServiceRequest → Deliverable`; payment simulated via price fields). See the [reconciliation log](../deliverables/doc-reconciliation-log.md) §D for the entity roster and the alignment task.
...
Most entities are read by a product screen. As of the schema-v2 simplification (28 entities total):
...
| ConnectedDeviceID | UUID | FK → ConnectedDevice | nullable. **Null = manual entry**; non-null points at the capture device — `ConnectedDevice.deviceType` disambiguates phone-sensor vs wearable capture without a parallel `dataSource` enum (dropped in v84) |
