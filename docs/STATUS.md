...
4. **Breadth build-out** (11–12 Jun, with Claude Code; order: profile → plans → social → experts → premium → dashboard → portals):
   ...
   - ✅ **Onboarding + Plans foundation COMPLETE** (12 Jun) — first-login wizard (#3: welcome → body metrics → training context → goal → plan generation), gated on new `profiles.onboarding_completed_at`. Plan generation per locked AI scope: Free → `BuildPlanSkeleton` (rule-based), Premium → `suggest-plan` Edge Function (stub, deployed; rule fallback). New: `FitnessPlan`/`PlannedWorkout` entities, `PlanGateway`, `GeneratePlan`/`CompleteOnboarding` controls; Train tab shows the active plan card (TODAY highlight + weekly chips). **91 tests** green.
