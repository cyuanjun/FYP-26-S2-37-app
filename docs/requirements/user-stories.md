...
| US25 | ✅ | As a registered free user, I want to join simple fitness challenges and earn badges so that I can stay motivated. | Challenges live (6 Jul): join/leave/create, live-computed leaderboards (challenge_leaderboards RPC); "badges" = XP/levels per design (SRS lag noted). **Shareable join codes added:** every challenge gets a unique server-assigned code (migration `20260713100000_challenge_join_code.sql`) — enter one in the Challenges-tab search field to open + join, or reveal/share it from the detail's invite popup (`FindChallengeByCode` control) |
...
| US48 | ⬜ | As an expert user, I want to upload or manage expert-related content so that I can provide useful fitness or wellness information to users. | Expert *content* upload/library **out of scope** (WBS has expert *services*, not a content library — pairs with US30). The expert *services* portal (#20–#24: listings, requests, deliverables, profile) **is** built — see US45–US51 |
...
| US63 | 🟨 | As a system admin, I want to manage the marketing website so that platform information, feature highlights, subscription details, and expert service information remain accurate. | `/admin/pricing` + `/admin/faq` (add/edit/reorder/hide) + `/admin/testimonials` moderation; featured experts/testimonials algorithm-ranked ([algorithms](../web/algorithms.md)); hero/feature media captured from the real app but swapped via files, not the portal |
