...
| Challenges | `CreateChallenge`, `JoinChallenge`, `LeaveChallenge`, `FindChallengeByCode` |
...
> - **Gateways:** the logical `SupabaseGateway` is realized as **seven feature gateways** — `ProfileGateway`, `WorkoutGateway`, `SocialGateway`, `FitnessGateway`, `PlanGateway`, `FeedbackGateway`, `DeviceGateway` (plus the as-designed `AuthGateway`, `AiGateway`, `WorkoutDataSource` incl. `WearableHrSource`/`CompositeWorkoutDataSource`, `SocialShareGateway`). `NotificationGateway` built 8 Jul (flutter_local_notifications + timezone; permission + one-shot zoned scheduling). `StorageGateway` built 9 Jul (avatars bucket; profile photos). `BleHeartRateSource` built 9 Jul (real GATT HR behind the `HrSource` abstraction; simulated `WearableHrSource` remains the mock-pairing/demo path).
...
> - **Challenge join codes added:** every challenge carries a unique, server-assigned, ambiguity-free 6-char code (`before insert` trigger + unique index; migration `20260713100000`). `FindChallengeByCode` (SEQ `find-challenge-by-code`) resolves a code entered in the Challenges-tab search field → opens the detail to review before joining. The code is hidden behind the detail's share icon → an invite popup (copy + system share via `SocialShareGateway.shareInvite`), and surfaced again right after `CreateChallenge`. Shared popup: `boundaries/ui/social/invite_code_dialog.dart`.
...
> - **Manual entry (US13) DESCOPED — removed:** `LogManualWorkout` + `ManualEntryScreen` were built 9 Jul then cut, because a free-text entry ran the same `end_workout_session` RPC as a captured session and let users farm XP/level/streak with no sensor evidence. All progress now requires a captured session (phone GPS / freeform / wearable). See reconciliation log §C8.
> - **Advanced analytics built (9 Jul):** #12.2 realized as a read-only Boundary (`AdvancedAnalyticsScreen`) over a pure ENTITY-rules module (`entities/advanced_analytics.dart` — ACWR, weekly buckets, Karvonen HR zones, personal bests; all deterministic + unit-tested). No new Control: it watches the existing `historyProvider`/`fitnessProfileProvider` read-side.
> - **Notification controls built (8 Jul):** `ScheduleReminders` (realizes the rule-based US19–21 alert engine — SEQ `schedule-reminders`): a pure `planReminders` rule function (unit-tested) + a `SyncReminders` control that replaces the OS schedule on shell load and #13.4 pref flips. Boundary additions: the #13.4 UPCOMING strip renders the last-synced plan.
