...
> - **Manual entry built (9 Jul):** `LogManualWorkout` (US13, SEQ `log-manual-workout`) — `ManualEntryScreen` off Train; reuses startSession (null device) → `end_workout_session` (now backdate-aware) → updateSummary, keeping every data-owned rule server-side.
