-- US13 manual workout entry was descoped/removed. Its backdating path (added in
-- 20260709090000) let a client pass p_metrics.started_at to end_workout_session
-- and backfill a past streak week via a direct RPC call - the exact farming
-- vector the descope aimed to close. This redefinition ignores any client start
-- time: the session keeps its real started_at and always ends at now(), so the
-- weekly-streak calc (keyed on ended_at) can no longer be backdated. Otherwise
-- identical to the live finalize path (XP, streak, level_up).
...
begin
  ...
  -- Client start time is ignored on purpose: ended_at is always now().
